...
class databasehandler():
    ...
    def init_db(self):
        ...
        if measurmenttable not in tables:
            c.execute("""create table {0} (timestamp datetime , pm2_5 float , pm10 float , deviceid integer)""".format(measurmenttable))
        conn.commit()
        conn.close()
    ...
    def add_measurement(self,measurement):
        ...
        c.execute("insert into {0} (timestamp,pm2_5,pm10,deviceid) values (?,?,?,?)".format(measurmenttable),linedata)
        conn.commit()
        conn.close()
